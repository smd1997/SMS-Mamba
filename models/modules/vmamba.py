...
class ConvLayer(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.,channels_first=False):
        ...
        self.conv1 = Conv2d_channel_last(in_features, hidden_features, 3, 1, 1)
        self.act = act_layer()
        self.conv2 = Conv2d_channel_last(hidden_features, out_features, 3, 1, 1)
...
class SS2Dm0:
    ...
    def forwardm0(self, x: torch.Tensor, **kwargs):
        if len(x.shape) <= 4:
            ...
            x = self.in_proj(x)
            if not self.disable_z:
                x, z = x.chunk(2, dim=-1) # (b, t, h, w, d)
                if not self.disable_z_act:
                    z = self.act(z)
            if self.with_dconv:
                x = self.conv3d(x) # (b, d, t, h, w)
            x = self.act(x)
            b, t, h, w, d = x.shape
            y = self.forward_core(x.reshape(b*t, h, w, d))
            y = self.out_act(y).reshape(b, t, h, w, d)
            if not self.disable_z:
                y = y * z
            out = self.dropout(self.out_proj(y))
            return out
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.mlp_branch:
            ...
            # self.mlp = FeedForward(dim=hidden_dim, ffn_expansion_factor=mlp_ratio)
...
if __name__ == "__main__":
    ...
    model = VSSM(
        depths=[2, 2, 4, 2], dims=16, drop_path_rate=0.2, 
        patch_size=4, in_chans=3, num_classes=1000, 
        ssm_d_state=64, ssm_ratio=1.0, ssm_dt_rank="auto", ssm_act_layer="gelu",
        ssm_conv=3, ssm_conv_bias=False, ssm_drop_rate=0.0, 
        ssm_init="v2", forward_type="m0_noz", 
