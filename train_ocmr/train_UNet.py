...
from utils.utils import SEED, kwargs, get_time, adjust_learning_rate, print_to_log_file
from pathlib import Path
from models.model_UNet_simple import TUNet
from tqdm import tqdm
from utils.dataset import OCMRDataset_coilmerged
from utils.Loss import Loss, compute_metrics
...
if __name__ == "__main__":
    ...
    parser.add_argument('-ep', type=int, default=1000, required=False, help='number of epochs')
    ...
    parser.add_argument('-cs', type=int, default=512, required=False, help='crop size')
    parser.add_argument('-coil', type=int, default=1, required=False, help='coil size')
    parser.add_argument('-d_model', type=int, default=512, required=False, help='d_model')
    ...
    parser.add_argument('-read_path', type=str, default='/mnt/data/dataset/OCMR_data', required=False, help='dataset path')
    ...
    if not args.eval:
        train_dataset = OCMRDataset_coilmerged(rootpath=OCMR_path, 
        ...
        train_dataloader = DataLoader(dataset=train_dataset,
                                    ...
                                    num_workers=4,
                                    pin_memory=True)
    test_dataset = OCMRDataset_coilmerged(rootpath=OCMR_path,
                        pre_name=args.pre_name, 
                        T=args.T, 
                        cutslice=True,
                        index=test_index,
    ...
    if kwargs['T'] != args.T:   kwargs['T'] = args.T
    if kwargs['hw'] != args.cs:     kwargs['hw'] = args.cs
    if kwargs['input_channels'] != args.T:
        kwargs['input_channels'] = args.T
        kwargs['num_output_channels'] = args.T
    model = TUNet(**kwargs)
    ...
    if args.ddp and torch.cuda.device_count()>=2:
        ...
        loss = Loss(DC=args.dcloss, device=device_id).to(device_id)
    else:
        device_id = device
        model = model.to(device_id)
        loss = Loss(DC=args.dcloss, device=device_id).to(device_id)
    ...
    if args.c or args.eval:
        ...
        start_ep = 0
        Cur_model_path = os.path.join(Cur_model_path, get_time()+args.pre_name)
        # if not os.path.exists(Cur_model_path):
    ...
    if args.eval:
        ...
        s = 0
        with torch.no_grad():
            for batch_idx, [input, label, kmask, uk] in enumerate(test_dataloader):
                output = model(input.to(device_id), kmask.to(device_id), uk.to(device_id))
                PSNR, SSIM = compute_metrics(output, label.to(device_id), device_id)
                p = p + PSNR
                s = s + SSIM
        ...
        for epoch in ep:
            ...
            s = 0
            loss_log = 0
            for batch_idx, [input, label, kmask, uk] in enumerate(train_dataloader):
                if scaler is not None:
                    with autocast():
                        output = model(input.to(device_id), kmask.to(device_id), uk.to(device_id))
                        optimizer.zero_grad()
                        l = loss(output, label.to(device_id), kmask.to(device_id), data_range=torch.tensor([1]).to(device_id))
                ...
                p = p + PSNR
                s = s + SSIM
            print_to_log_file(log_file, "%s Epoch %d lr %.6f train_loss: %.4f " % (get_time(), epoch, cur_lr,  float(loss_log/(batch_idx+1))))
            ...
            if float(p/(batch_idx+1)) > best_psnr:
                ...
                'model_state_dict': model.state_dict(),
                'optimizer_state_dict': optimizer.state_dict()
                }, os.path.join(Cur_model_path, 'model_best_psnr_epoch{}.pth'.format(epoch)))     
