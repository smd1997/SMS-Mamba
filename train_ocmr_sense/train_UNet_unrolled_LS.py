...
if __name__ == "__main__":
    ...
    model = UNet_unrolled_LS(iter=args.iter, DC_type=args.dctype, kwargs=kwargs)
    ...
    if args.ddp and torch.cuda.device_count()>=2:
        ...
        device_id = device
        model = model.to(device_id)
    loss = Loss(num_channel=1, device=device_id).to(device_id)
    ...
    if args.c or args.eval:
        ...
        start_ep = 0
        Cur_model_path = os.path.join(Cur_model_path, get_time()+args.pre_name+'_unet_unrolled_LS_'+args.dctype)
            ...
            for batch_idx, datalist in enumerate(train_dataloader):
                ...
                if scaler is not None:
                    with autocast():
                        output = model(datalist)
                        optimizer.zero_grad()
                        l = loss(output, labeli, data_range=torch.tensor([1]).to(device_id))
